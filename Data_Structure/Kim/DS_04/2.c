...
int main(void)
{
	term a[MAX_TERMS]; // transpose 전
	term b[MAX_TERMS]; // transpose 후의 정보 를 담을 구조체
	int i, numTerms;
	FILE *fpA;
	FILE *fpB;

	fopen_s(&fpA, "a.txt", "r"); // a.txt는 입력 파일
	fopen_s(&fpB, "b.txt", "w"); // b.txt는 출력 파일

	// input a.txt
	fscanf_s(fpA, "%d%d%d", &a[0].row, &a[0].col, &a[0].value);
	numTerms = a[0].value; // 첫번째 value => the total number of nonzero entries
