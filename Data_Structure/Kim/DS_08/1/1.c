...
void insert(listPointer* first, listPointer x, int data_f); 
void delete(listPointer* first, listPointer trail, listPointer x);
void locate(listPointer first, listPointer* x, int data_f); // 정렬을 위해 위치를 잡아줄 함수
...
void locate(listPointer first, listPointer* x, int data_f)
{
	listPointer p1 = first, p2 = first;
	if (first == NULL) {
		*x = first;
	}
	else
